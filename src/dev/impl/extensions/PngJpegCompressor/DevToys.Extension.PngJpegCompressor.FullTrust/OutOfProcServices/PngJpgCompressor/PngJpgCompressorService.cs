...
namespace DevToys.OutOfProcService.OutOfProcServices.PngJpgCompressor
{
    ...
    {
        protected override Task<PngJpgCompressorWorkResultMessage> ProcessMessageAsync(PngJpgCompressorWorkMessage inputMessage, CancellationToken cancellationToken)
        {
            return Task.FromResult(new PngJpgCompressorWorkResultMessage
            {
                ErrorMessage = null,
                NewFileSize = 123,
                PercentageSaved = 1,
                TempCompressedFilePath = "test"
            });
            //Debugger.Launch();
